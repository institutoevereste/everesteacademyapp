// Aguardar carregamento do Firebase do index.html
const waitForFirebase = () => {
    return new Promise((resolve) => {
        if (window.Firebase) return resolve(window.Firebase);
        window.addEventListener('firebase-ready', () => resolve(window.Firebase));
...
// Iniciar App apenas quando Firebase estiver pronto
(async function initApp() {
    const FB = await waitForFirebase();
    const {
      initializeApp, getAuth, signInWithEmailAndPassword, signOut, onAuthStateChanged, 
      signInAnonymously, signInWithCustomToken, getFirestore, doc, getDoc, setDoc, 
      addDoc, updateDoc, deleteDoc, collection, query, where, getDocs, onSnapshot, 
      serverTimestamp
    } = FB;
